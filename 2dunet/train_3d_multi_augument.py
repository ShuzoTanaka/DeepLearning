...
class Nifti3DDataset(Dataset):
    ...
    def __init__(
        ...
        dura_label: int = 2,
        augment: bool = False,  # ★追加：学習時のみ True
    ):
    ...
    def apply_augment(self, img, rmask, dmask):
        ...
        # 3) 輝度スケーリング
        if np.random.rand() < 0.5:
            scale = 0.9 + 0.2 * np.random.rand()  # 0.9–1.1
            img = img * scale
        ...
        # ★ここで必ず float32 & positive stride にする
    ...
    def __getitem__(self, idx: int):
        ...
        # ★ from_numpy 前に必ず copy された float32
        img_tensor = torch.from_numpy(img)  # → FloatTensor
        mask_tensor = torch.from_numpy(mask)  # → FloatTensor
...
def main():
    ...
    val_ds = Nifti3DDataset(
        ...
        nerve_root_label=args.nerve_root_label,
        dura_label=args.dura_label,
        augment=False,  # ★Validationでは必ず False
    )
    ...
    # ============================
    # test (best model で nerve-root Dice を計算 & .txt 保存)
    # ============================
