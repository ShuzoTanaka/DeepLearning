...
# クロップ範囲（X, Y）
CROP_X = (50, 200)  # 50 <= x < 200
CROP_Y = (45, 210)  # 45 <= y < 210
...
class Nifti3DDataset(Dataset):
    ...
    - 50 < x < 200, 45 < y < 210, z は全範囲でクロップして使用
    """
    ...
    def __getitem__(self, idx: int):
        ...
        img = img[x_min:x_max, y_min:y_max, :]  # z 全範囲
        ...
        nerve_mask = (lab == self.nerve_root_label).astype(np.float32)
        nerve_mask = nerve_mask[None, ...]
...
def center_crop_3d_to_match(a: np.ndarray, b: np.ndarray):
    ...
    return crop(a, d_t, h_t, w_t), crop(b, d_t, w_t, w_t)
...
def pair_all_paths_flat(dataset_root: str) -> Tuple[List[str], List[str]]:
    ...
    if not (os.path.exists(img_dir) and os.path.exists(lab_dir)):
        raise RuntimeError("images / labels フォルダが見つかりません")
...
def main():
    ...
    parser.add_argument(
        "--cv",
        action="store_true",
        help="Dataset/images & Dataset/labels から K-fold 交差検証",
    )
    parser.add_argument(
        "--num_folds",
        type=int,
    ...
    # ========================
    if args.cv:
        print("=== K-fold Cross Validation モード ===")
        ...
        for fold_idx, val_idx in enumerate(folds):
            ...
            print(f"#Train cases (fold {fold_idx}): {len(train_imgs)}")
            print(f"#Val   cases (fold {fold_idx}): {len(val_imgs)}")

            train_ds = Nifti3DDataset(
                train_imgs, train_labs, nerve_root_label=args.nerve_root_label
            )
            val_ds = Nifti3DDataset(
                val_imgs, val_labs, nerve_root_label=args.nerve_root_label
            )
            ...
            best_path = os.path.join(
                args.out_dir, f"fold{fold_idx}_best_3dunet_nerve.pth"
            )
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                if val_loss < best_val:
                    ...
                    if epochs_no_improve >= patience:
                        print(
                            f"Early stopping in fold {fold_idx}: no improvement in val_loss "
                            f"for {patience} consecutive epochs."
                        )
                        break

            # fold の val を test とみなして Dice 計算
            print(f"\n[Fold {fold_idx}] Evaluate on validation set (as test)")
            ...
            print(f"[Fold {fold_idx}] Mean Dice: {fold_mean:.4f}")

            # foldごとの結果を txt に保存
            fold_txt_path = os.path.join(args.out_dir, f"fold{fold_idx}_dice.txt")
            lines = []
            lines.append(f"Fold {fold_idx} case-wise Dice (nerve root only)")
            for cid, d in case_dice.items():
                lines.append(f"{cid}: {d:.4f}")
            lines.append(f"Fold {fold_idx} mean Dice: {fold_mean:.4f}")

            with open(fold_txt_path, "w") as f:
                f.write("\n".join(lines))

            print(f"[Fold {fold_idx}] Saved Dice results to {fold_txt_path}")

        # 全 fold のサマリ
        fold_mean_dice_arr = np.array(fold_mean_dice_list, dtype=np.float32)
        print("\n===== Cross Validation Summary =====")
        for k, d in enumerate(fold_mean_dice_list):
        ...
        lines = []
        lines.append("K-fold Cross Validation Summary (nerve root Dice)")
        for k, d in enumerate(fold_mean_dice_list):
    ...
    val_ds = Nifti3DDataset(val_imgs, val_labs, nerve_root_label=args.nerve_root_label)
    test_ds = Nifti3DDataset(ts_imgs, ts_labs, nerve_root_label=args.nerve_root_label)
