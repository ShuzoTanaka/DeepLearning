...
from scipy.ndimage import zoom  # ★ 追加
import scipy.ndimage as ndi  # ★追加
...
def resample_to_isotropic(
    ...
    """
    img: (X,Y,Z) float32
    lab: (X,Y,Z) int16 or None
    orig_spacing: (sx, sy, sz) [mm]
    target_spacing: (tx, ty, tz) [mm]

    物理サイズを保ったまま、target_spacing にリサンプリングする。
    画像: order=3 (tricubic 相当)
    ラベル: order=0 (nearest neighbor)
    """
    ...
    # 画像: 3次補間
    img_iso = ndi.zoom(img, zoom=zoom_factors, order=3)
    img_iso = img_iso.astype(np.float32)

    if lab is not None:
        # ラベル: 最近傍補間でラベル値を保つ
        lab_iso = ndi.zoom(lab, zoom=zoom_factors, order=0)
        lab_iso = lab_iso.astype(np.int16)
...
class Nifti3DDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # ★ 元の voxel サイズ (sx, sy, sz) を NIfTI header から取得
        orig_spacing = img_nii.header.get_zooms()[:3]  # 例: (1.0, 1.0, 1.5)

        # ★ 等方リサンプリング (ここでは 1.0mm isotropic)
        img, lab = resample_to_isotropic(
            img, lab, orig_spacing=orig_spacing, target_spacing=(1.0, 1.0, 1.0)
        )
...
def main():
    ...
    # ===== ここから：結果を .txt で保存 =====
    # .pth が置いてあるフォルダ
    ckpt_dir = os.path.dirname(best_path)
    txt_path = os.path.join(ckpt_dir, "dice_nerve_cubic.txt")

    lines = []
    lines.append("=== Case-wise Dice (nerve root only, cubic) ===")
    for cid, d in case_dice.items():
        lines.append(f"{cid}: {d:.4f}")

    dice_values = np.array(list(case_dice.values()), dtype=np.float32)
    mean_dice = float(dice_values.mean())
    lines.append(f"Mean Dice (nerve root, cubic): {mean_dice:.4f}")
